		       ...
		       xdr::reply_cb<Status> cb) // We already make the pointers unique
{
  ...
  // Setting up outbound TCP
  std::cerr << "connecting to PORT: " << XDRDEMO_PORT_THREE << std::endl;
  xdr::unique_sock fd =
    xdr::tcp_connect("localhost", std::to_string(XDRDEMO_PORT_THREE).c_str()); // basic socket for tcp
  xdr::pollset ps_out;
  xdr::rpc_sock s3(ps_out, fd.release());

  xdr::arpc_client<KVPROT3> client3(s3); // async rpc with application version feeing it the fd and ps we had. Inittializes client

  // Issue the operation:
  client3.kv_put_3(Key("a"), Value("a"), // Change this later.
		  [](xdr::call_result<Status> res) {
		    exit(0);
		  });
...
void
KVPROT3_server::kv_null_3(xdr::reply_cb<void> cb)
{
  cb(); // return void 
}

void
KVPROT3_server::kv_put_3(std::unique_ptr<Key> k, std::unique_ptr<Value> v,
		       xdr::reply_cb<Status> cb) // We already make the pointers unique
{
  std::string& path = cb.get_path();
  std::clog << "[kv_put] cb path: " << cb.get_path() << std::endl;
...
void
KVPROT3_server::kv_get_3(std::unique_ptr<Key> k, xdr::reply_cb<GetRes> cb) // Use 
{
  std::string& path = cb.get_path();
  std::clog << "[kv_get] cb path: " << cb.get_path() << std::endl;
  path = "G" + path;
...
main(int argc, char **argv)
{
  ...
  if (atoi(argv[1]) == 1) {
    ...
    xdr::pollset ps;
    xdr::unique_sock sock = xdr::tcp_listen(std::to_string(XDRDEMO_PORT_THREE).c_str());
    xdr::arpc_tcp_listener<> listen(ps, std::move(sock), false, {}); // async rpc lister
    KVPROT3_server s;
    std::cerr << "Server instance " << s.id <<  " listening on: " << XDRDEMO_PORT_THREE <<  std::endl;
    listen.register_service(s); 
    ps.run();
  }
